...
- **실시간 요약 정보 제공**: 하단 3열 레이아웃을 통해 주요 지표 가시화.
- **설문 조사**: 현재 참여해야 할 미응답 설문 개수 실시간 표시.
- **자원 예약**: 
  - `오늘 이용중`: 현재 사용 중인 자원/공간 수 및 총 예약 건수 요약.
  - `승인 대기`: 내가 관리자인 자원에 들어온 새로운 예약 요청 건수 알림.
- **그룹**: 소규모 커뮤니티(스터디, 프로젝트) 구성을 위한 확장 공간 확보.
...
#### 1. **파일 업로드 및 스토리지 관리 시스템 구축**
   - **조직별 용량 제한 기능**:
     - 조직마다 독립적인 파일 업로드 제한 설정 (3MB / 5MB / 10MB / 차단).
     - 최고 관리자가 조직별로 용량 제한을 설정하고 변경 이력 추적.
   - **실시간 스토리지 사용량 추적**:
     - 파일 업로드 시 자동으로 조직의 총 파일 수와 사용 용량을 Firestore에 기록.
     - Atomic Increment를 활용한 정확한 통계 집계.
   - **통합 대시보드 추가**:
     - 최고 관리자: 전체 플랫폼 스토리지 사용량 카드 추가.
     - 조직 관리자: 우리 조직의 업로드 설정 및 현황을 첫 화면 카드로 표시.
   - **파일 압축 최적화**:
     - 클라이언트 측에서 이미지를 자동 압축 (Canvas API 활용).
     - 최대 1920x1080 해상도, JPEG 70% 품질로 변환.
     - 원본보다 압축 파일이 클 경우 원본 사용.
   - **적용 범위**:
     - 공지사항 첨부파일 (`/notice`, `/admin/super/notices`, `/groups/[groupId]`).
     - 설문조사 파일 응답 (`/surveys/[id]`).

#### 2. **UI/UX 표준화 및 개선**
   - **날짜 형식 통일**:
     - 전체 시스템에 "YYYY. MM. DD.(요일)" 형식 적용.
     - `dateUtils.ts` 모듈로 중앙 관리.
   - **탭 스타일 통일**:
     - 최고 관리자 및 조직 관리자 페이지의 탭 네비게이션을 Pill 스타일로 통일.
   - **아이콘 업데이트**:
     - 즐겨찾기: 🔗 → ⭐
     - 그룹 관리: 📂 → 👥
     - 회원 관리: 👥 → 👤
...
#### 3. **자동화 및 효율성 개선**
   - **시스템 데이터 자동 최적화**:
     - 조직 관리자 센터 접속 시 자동으로 삭제된 자원의 잔여 예약 데이터 정리.
     - 수동 실행 UI 제거 (백그라운드 자동 실행).
   - **정책 탭 정리**:
     - 최고 관리자의 "정책" 탭 숨김 처리 (향후 확장 대비).
     - 데이터 마이그레이션 도구 제거 (완료된 기능).

#### 4. **코드 모듈화 및 유지보수성 향상**
   - **유틸리티 함수 분리**:
     - `dateUtils.ts`: 날짜 포맷팅 전담.
     - `fileUtils.ts`: 이미지 압축 및 파일 처리 전담.
   - **일관된 데이터 구조**:
     - 조직 문서에 `uploadLimit`, `storageUsage`, `limitHistory` 필드 추가.
     - 파일 업로드 로직을 각 모듈에서 동일한 방식으로 구현.
